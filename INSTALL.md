...
Fully-local control of Haier ACs that pair with the **Haismart** (Haier U+/uHome SEâ€‘Asia) app â€” no cloud at
...
- **A Haismart account.** Easiest is an **email/phone + password** account. If you sign in with **Google/
  ...
  the app, and log in with that account â€” sharing grants the same local access as ownership.
- `cryptography` and `httpx` are already shipped with HA â€” nothing else to install.

## 1. Install

### HACS (recommended)

In **HACS â†’ â‹® â†’ Custom repositories**, add `https://github.com/cantruchd/haismart` with category
**Integration**. Install **Haismart**, then **restart Home Assistant**. The component is
...
`<config>/custom_components/` and restart Home Assistant. It's self-contained â€” nothing to pip-install.
...
You can't easily pip-install into the core container here â€” this is the one awkward case. For testing,
...
### Options
- `--symlink` â€” symlink the component instead of copying (edits apply on the next HA restart; handy for dev).
- `--copy-only` â€” skip the pip step (only if the two libs are already importable in HA's env).
- `PYTHON=<exe>` â€” same as `--python`.

Then **restart Home Assistant.**

## 2. Add the integration

**Settings â†’ Devices & Services â†’ Add Integration â†’ "Haismart"** (it may also appear on its own
via zeroconf, since it listens for `_cae._udp`). Pick one path:

- **Login (recommended) â€” no key to paste:** email/phone + password + the **country your Haier
  ...
  choose it **fetches the localKey from the cloud automatically** â€” you never paste a key â€” and **finds the
...
add the first, run **Add Integration â†’ Haismart** again â€” the picker now shows only the AC(s) you haven't
...
sensors. Change the setpoint â€” the AC applies it and the card confirms from the AC's own reply immediately.

## 4. Optional: go fully cloud-independent (block Haier's servers)

**Everything already runs locally after setup** â€” reads and control never touch Haier. The *only* runtime
cloud dependency is that Haier's server can **rotate the per-device `localKey`** (which the integration then
re-fetches). If you'd rather the units never phone home at all, you can block Haier's cloud â€” then the key
...
> âš ï¸ **The most common way this goes wrong:** reaching for a router feature called *MAC filtering* or
> *IP/port filtering*. Those usually remove the device from the **network altogether** â€” it disappears
...
> **Before blocking:** note each AC's current key/version â€” from HA go **Settings â†’ Devices & Services â†’
> Haismart â†’ the device â†’ Diagnostics** (redacted), or run `probe_localkey_version(ip, deviceId)`. Keep a
> copy as your escape hatch; you can always re-add via the **manual** path (host + deviceId + key), no cloud.

### Option A â€” block the gateway address (recommended)

Add a firewall rule denying traffic **to `43.156.75.60`** â€” Haier's MQTT gateway
...
off your LAN â€” which is the failure mode below.

If you apply it LAN-wide, note that Home Assistant also loses its ability to fetch a fresh key. That
is harmless here â€” a blocked AC never rotates its key, so there is nothing to fetch â€” but it does mean
you should lift the rule before deliberately re-keying anything.

### Option B â€” per-device WAN block (only if your router really separates WAN from LAN)
...
> âš ï¸ **Check what your router's feature actually blocks.** Router options named *MAC filtering*,
> *IP filtering* or *IP/port filtering* frequently block the device from the **network entirely**, not
> just from the internet â€” the AC drops off your LAN, Home Assistant loses it, and local control stops.
...
> everything else â€” order matters.
...
### Not recommended â€” DNS blocking
...
Haier, but do not use it as your isolation mechanism â€” and if you do use it, verify with the sensor
...
1. **Watch the AC's own `Cloud connection` sensor** (diagnostic, one per device). This is the direct
   ...
   sensor reads **off**. Give it ~2 minutes â€” the AC only notices once a keepalive expires, and its
   `raw_state` attribute passes through `retrying` before settling on `disconnected`. (Coming back is
   faster: about 10 seconds.) If it stays **on**, the AC is still getting out â†’ use Option B.
2. Confirm local read/control still works right after blocking (some IoT gear sulks without cloud â€” these
...
3. Confirm the key stops rotating. A cut-off unit does **not** rotate at all. So a `Local key` version
   that hasn't moved in a day is corroboration â€” but the sensor in step 1 is the signal, since a quiet
...
| | |
|---|---|
| Reading and control over `:56800` | **unaffected** â€” setpoint, mode, fan, switches all work normally |
| The stored local key | stays valid; a cut-off unit does not rotate it |
| The `Cloud connection` sensor | reports `off` â€” this is the state you want |
...
"What if Haier discontinues the service?" â€” a device you **already control keeps working**. The localKey is
...
1. **Archive every AC's localKey now** (while the cloud is alive). Each AC has a **Local key** diagnostic
   sensor (disabled by default). Enable it on the device page â†’ its state is the key and its attributes
   carry host + deviceId + version + model ID. It then rides along in your HA backups automatically. Keep
   those safe â€” the key grants ongoing local control. (The **Model ID** sensor shows the same identifier
...
2. **Onboard each AC via the config-flow `manual` path** (host + deviceId + key from the backup). Manual needs
   **zero cloud** â€” no login, no gateway â€” so nothing depends on Haier being up. The model ID it needs to
...
3. **Firewall the ACs** (Â§4) so the key never rotates.

That's it â€” those ACs are now Haier-independent. **The one thing this doesn't cover** is *factory-resetting* or
...
- **"No decodable status" / entities unavailable right after adding.** Two different causes, and
  ...
  â€” the key is fine; please report the model (see [`docs/new-model.md`](docs/new-model.md)).
  Otherwise it is a **stale `localKey`** â€” it
  ...
  Each failed cycle then logs which cause it is â€” the wording tells them apart:

  | Log says | Means |
  |---|---|
  | `nothing decrypted this cycle` | the key is wrong/stale, **or** the AC pushed no status at all |
  | `localKey is good â€¦ unrecognised frame` | the key is fine, but what the AC pushed isn't a status report (no `2715` signature, or too short) |
  ...
  line includes the frame â€” please open an issue with it if the entity stays unavailable. It carries
...
- **The AC changed IP address.** Handled automatically: after a failed read the integration
  ...
  and updates the entry to follow it â€” usually within the same poll, so you see nothing at all. A
...
- **Can't reach the AC.** Confirm HA and the AC are on the same subnet and `:56800` is open:
  ...
  provide; if you blocked the AC's WAN (Â§4), make sure you left its **LAN** open.
- **Login rejected.** The integration now names the likely cause rather than listing all three
  fields. "No Haier account â€¦ in the country you selected" (retCode 30032) means the **country** is
  ...
  appear, the account has none bound â€” share the AC to it in the app first.
