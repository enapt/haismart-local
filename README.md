# Haismart â€” Haier air conditioners in Home Assistant, with no cloud
...
once so the integration can fetch your unit's key â€” after that it talks only to the AC, on your LAN._

**ðŸŒ Getting started in your language:** [Bahasa Indonesia](docs/i18n/README.id.md) Â·
[à¹„à¸—à¸¢](docs/i18n/README.th.md) Â· [Tiáº¿ng Viá»‡t](docs/i18n/README.vi.md) Â·
[Bahasa Melayu](docs/i18n/README.ms.md)\
_The integration's own screens are translated into 30 languages â€” including Urdu, Hindi, Bengali,
Tamil, Arabic and Persian â€” and follow whatever language you've set in Home Assistant. These docs are
...
> Reading state and sending commands never leave your network â€” and keep working if your internet
> does not.
...
work fine â€” this is used daily on an account registered outside South-East Asia.

| Your app | Supported here? | Use instead |
|---|---|---|
| **Haier / Haismart / Haier U+ / uHome** | âœ… **Yes** | â€” |
| hOn (mostly Europe) | âŒ No â€” these modules don't open port 56800 at all | [Andre0512/hon](https://github.com/Andre0512/hon) |
| Haier æ™ºå®¶ (mainland China) | âŒ No â€” different cloud | [banto6/haier](https://github.com/banto6/haier) |
| SmartHQ (US / GE Appliances) | âŒ No â€” different platform entirely | â€” |
| SmartAir2 / Smart Clima (older units) | âŒ No â€” same port, older unencrypted protocol | [oxystin/homebridge-haier-air-conditioner](https://github.com/oxystin/homebridge-haier-air-conditioner) |

**Confirmed working units** are listed in [`DEVICES.md`](DEVICES.md). Yours not there? It will very
likely still work â€” the integration builds itself from the model description your AC's own cloud
...
| **Energy** | A running kWh total, on the units that keep one themselves. Goes straight on the Energy dashboard â€” no helper needed. Most units carry the register but never fill it in, and there the sensor stays unavailable; see [Energy monitoring](#energy-monitoring) |
...
| **Self-clean** *(diagnostic, on/off)* | Whether a self-clean cycle is running. Reported, not controlled â€” the cycle runs to completion and ignores a second press, so there is nothing an off switch could do |
| **Optional features** *(diagnostic, on/off)* | The extra functions a unit reports having â€” fresh air, electric heating, a 10 Â°C keep-warm, ambient light, an intelligent mode, humidification, the buzzer, and others â€” each read-only, and only the ones your unit *actually* has (a model over-declares; the ones it lacks are hidden) |
...
| **Fault** *(diagnostic, problem)* | Whether the unit is reporting a fault. Its attributes name the active faults with the service code the unit shows (E1, F4, â€¦) â€” that is what an engineer will ask for |
...
| **Model ID** *(diagnostic)* | The identifier that selects your unit's report layout. Shown shortened (it's 64 characters); the exact value is on the entity's `uplus_id` attribute â€” quote that in a bug report about a model that isn't decoded |
| **Cloud connection** *(diagnostic, on/off)* | Whether the AC itself can still reach Haier's servers â€” see [going fully cloud-independent](#going-fully-cloud-independent) |
| **Local key** *(diagnostic, off by default)* | Your unit's key, so it rides along in HA backups |

Which of these appear depends on your model â€” the integration only exposes controls it can actually
...
settings in certain states â€” a unit in fan-only discards the temperature you set, and most of them
...
keeps only its power and mode controls. Nothing is hidden while the AC is merely switched off â€” that
is when you are most likely to be setting it up.

The climate entity also carries **presets** for the three comfort modes â€” eco, sleep and boost â€” so
...
them â€” "Position 1" is the first stop it offers. A unit that publishes only fixed and auto for an
axis gets no select for it, since the swing control already says everything there is to say.

Not everything the air conditioner reports becomes an entity â€” its **firmware version**, for
...
records into Home Assistant's history and long-term statistics on its own â€” but the **Energy
...
to the Energy dashboard under **Settings â†’ Dashboards â†’ Energy â†’ Individual devices**. It is the
...
register and never fills it in â€” read on.

To build a total from the power reading instead, add a Riemann-sum integral helper over it:

1. **Settings â†’ Devices & services â†’ Helpers â†’ Create helper â†’ Integral sensor**
2. Pick your AC's **Power** sensor as the input
3. Metric prefix **k** (kilo), time unit **hours** â€” that gives you kWh
4. Method: **Trapezoidal** is the sensible default for a value that ramps

Then add the resulting kWh sensor under **Settings â†’ Dashboards â†’ Energy â†’ Individual devices**.
...
- **Check the helper's state class is `total_increasing`.** If the Energy dashboard will not offer
  your new sensor, this is almost always why â€” a helper left on `total` can also produce spikes in
...
- **It is an estimate, and so is the manufacturer's.** The figure comes from the unit's own current
  ...
  The vendor app's energy screens are estimates too â€” by their own wording they are "based on the
...
integration's **Configure** menu. One poll fetches everything in a single connection â€” status,
faults and the power figures â€” because these units accept only one connection at a time.

If you want a different rhythm than a fixed interval, Home Assistant has a documented way that works
for any integration: open the integration's **â‹® â†’ System options** and turn off *Enable polling for
...
card reflects the change at once instead of waiting for the next poll. The engineering readings â€”
power, current, frequency, the coil and discharge temperatures, compressor and fan â€” are not part of
...
- Installing this **does not stop your AC talking to Haier**. It keeps its own cloud connection
  unless you firewall it â€” see [going fully cloud-independent](#going-fully-cloud-independent).
...
- Social logins (Google / Facebook) have no password to sign in with. Create a throwaway
  email/password Haier account, **share the AC to it** in the app, and use that here â€” sharing grants
...
1. **Restart Home Assistant.** A custom integration's code is only loaded at startup â€” reloading the
   entry is not enough.

<details>
<summary>The button didn't work â€” add it by hand</summary>

1. Open **HACS** from the sidebar.
1. Three-dot menu, top right â†’ **Custom repositories**.
1. Repository: `https://github.com/cantruchd/haismart`, type **Integration** â†’ **Add**.
1. Search HACS for **Haismart** â†’ **Download**.
1. Restart Home Assistant.

</details>

<details>
<summary>Option 2 â€” manual installation</summary>
...
It's fully self-contained â€” no `pip install` step, the helper libraries are bundled.
...
Or: **Settings â†’ Devices & Services â†’ + Add Integration â†’ Haismart**. If it isn't listed, hard-refresh
...
one's key automatically, and finds it on your network â€” you won't paste anything.

> The country field is the **phone dialling code of the country your Haier account was created in**
> â€” not where the AC is installed, and not necessarily where you live now. Getting it wrong is the
...
**Manual.** Host + device ID + local key, entered directly. Completely offline â€” no account needed.
...
1. **Archive the key first.** Enable the *Local key* diagnostic sensor on the device page â€” its state
...
2. **Block the AC's internet access** at your router â€” the simplest rule that works is denying traffic
   ...
   features called "MAC filtering" or "IP filtering" â€” they often cut the device off your LAN entirely
...
4. **Check that it worked.** Each AC has a **Cloud connection** diagnostic sensor. It asks the AC
   itself â€” over a local, unauthenticated query that never contacts Haier â€” whether it can still
...
Handled for you. If a poll fails, the integration looks the unit up by its device ID â€” which is the
Wi-Fi module's MAC â€” finds where it has moved to, and updates itself to follow, usually within the
...
right, check whether your account is actually a Haier / Haismart one â€” hOn and Haier China accounts
...
- **Stale local key** â€” keys rotate server-side. If you signed in with your account, it re-fetches
  automatically; otherwise you'll be prompted to re-authenticate.
- **A report layout we don't know yet** â€” your model packs its status differently. The integration
  ...
  the likely answer for you â€” it carries a ranked list of candidate layouts with the values each
  one decodes. See [`docs/report-layouts.md`](docs/report-layouts.md).

</details>

<details>
<summary><b>Temperatures show the wrong unit (Â°F instead of Â°C, or vice versa)</b></summary>
...
history. So sensors added before the change keep the old unit while newer ones use the new one â€” it
applies to any integration's temperature sensors, not just this one.

Two ways to fix it, per sensor:

- **Keep history (recommended):** open the sensor â†’ its settings (cog) â†’ **Unit of Measurement** â†’
...
An address change is not usually the cause â€” the integration follows a unit that moves. Check that
...
For a quick question â€” "is my model likely to work?", "does this log line mean what I think?" â€” there
...
1. Download diagnostics: **Settings â†’ [Devices & Services](https://my.home-assistant.io/redirect/integrations/)
   â†’ Haismart â†’ â‹® â†’ Download diagnostics**. Secrets are redacted; the raw status bytes it contains
...
attribute your unit declares, read off its own report** â€” the settings that have no entity as well as
the ones that do â€” alongside the values your air conditioner publishes through its cloud profile, so
...
writing any code â€” see [`docs/new-model.md`](docs/new-model.md) for a short capture procedure. When a
...
- **Report a new model** â€” the capture procedure in [`docs/new-model.md`](docs/new-model.md) turns
  adding a model into a desk job. No hardware access needed on our side.
- **Translations** â€” the UI strings live in
  [`translations/`](packages/ha-haismart/custom_components/haismart/translations). Adding a language
  is a single JSON file.
- **Code** â€” see [`CONTRIBUTING.md`](CONTRIBUTING.md). It's a three-package monorepo; tests run with
...
The local uSS/HRDP protocol support here â€” the handshake, the AES/localKey biz-data layer, the status
decode and the grSetDAC control path â€” was worked out from scratch for this project, for
...
- [bstuff/haier-ac-remote](https://github.com/bstuff/haier-ac-remote) and
  [roeij/py-haier-ac-remote](https://github.com/roeij/py-haier-ac-remote) â€” early port-56800 work
- [KoalaBear84/HaierAC](https://github.com/KoalaBear84/HaierAC) â€” protocol logging, session behaviour
- [oxystin/homebridge-haier-air-conditioner](https://github.com/oxystin/homebridge-haier-air-conditioner)
  â€” SmartAir2 units and a valuable compatibility list
- [paveldn/haier-esphome](https://github.com/paveldn/haier-esphome) â€” the e++ frame layer
- [banto6/haier](https://github.com/banto6/haier) â€” the mainland-China cloud
- [Andre0512/hon](https://github.com/Andre0512/hon) / [pyhOn](https://github.com/Andre0512/pyhOn) â€”
  the hOn platform

And to everyone who opens an issue, reports a model, or stars the repo. â­
...
is no authentication bypass, no defeated protection and no per-user secret of anyone else's involved â€”
...
supply your own, every one of them is overridable by environment variable â€”
...
project so a unit you own can be driven from your own network. That is the point of the exercise â€”
